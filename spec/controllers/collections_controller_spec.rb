...
RSpec.describe CollectionsController, type: :controller do
  ...
  describe "GET #index" do
    it "responds successfully with an HTTP 200 status code" do
      sign_in
      # Collection.create(name: "test", user_id: current_user.id)
      get :index
    ...
    it "loads all of the posts into @collections" do
      sign_in
      collection1, collection2 = Collection.create(name: "test1", user_id: 1), Collection.create(name: "test2", user_id: 1)
      get :index
  ...
  describe "GET #show" do
    it "responds successfully with an HTTP 200 status code" do
      sign_in
      collection = Collection.create(name: "test", user_id: 1)
  ...
  describe "POST #create" do
    it "responds successfully with a redirect to collections" do
      sign_in
      post :create, {:collection => {name: "test", user_id: 1}}
   ...
   #    expect(response).to have_http_status(200)
    ...
    it "responds to failure with rendering the new form" do
      sign_in
      post :create, {:collection => {name: nil, user_id: 1}}
      expect(response).to render_template("new")
    end
